...
public:
	...
	void addTo(set<string> &src, set<string> &des)
	{
		set<string> ::iterator iterJ = src.begin();
		for (; iterJ != src.end(); iterJ++)//将其加入到first集合
		{
			des.insert(*iterJ);
		}
	}
	bool first(string str)//求一个变元的first集合，返回该非终结符是否可空
	{
		bool flag = false;
		if (!firstSet[str].empty())//已经存在,不重复求
		{
			set<string>::iterator iter = firstSet[str].find("$");//查找是否可空
			flag = (iter != firstSet[str].end());
			return flag;
		}
		for (int i = 0; i < (int)gram->P.size(); i++)
		{
			if (str == gram->P[i].genL)//找到第i组产生式左部
			{
				for (int j = 0; j < (int)gram->P[i].genR.size(); j++)//第i组产生式的第j个
				{
					...
					else if (gram->isTermin(gram->P[i].genR[j][0]))//是终结符或
					{
						firstSet[str].insert(gram->P[i].genR[j][0]);
					}
					else if (gram->isNon(gram->P[i].genR[j][0]) && gram->P[i].genR[j][0] != str)//是非终结符且不重复
					{
						...
						addTo(firstSet[gram->P[i].genR[j][0]], firstSet[str]);//将右部first添加到当前first集合
						...
						{//递归求解，若该非终结符可空则继续求下一个非终结符的first
							...
							else//如果是非终结符
							{
								...
								addTo(firstSet[gram->P[i].genR[j][k]], firstSet[str]);
							}
						}
					...
					if (gram->P[i].genR[j][k] == str)
					{
						if ((k + 1) == (int)gram->P[i].genR[j].size())//是最后一个符号且左部不重复
						{
							if (gram->P[i].genL != str)
							{
								...
								addTo(followSet[gram->P[i].genL], followSet[str]);//将左部的follow集合加入到当前follow集合
							}
							else continue;
						}
						...
						else if (gram->isNon(gram->P[i].genR[j][k + 1]))//后一个符号是非终结符,需要求解后面一串符号的first集合
						{
							...
							for (int t = k + 1; t < (int)gram->P[i].genR[j].size(); t++)
							{
								...
								{
									isEmpty = first(gram->P[i].genR[j][t]);
									addTo(firstSet[gram->P[i].genR[j][t]], followSet[str]);//将当前字符首符号添加到当前follow集合
									followSet[str].erase("$");
								}
								if (!isEmpty)//非空
								{
							...
							{
								follow(gram->P[i].genL);//求左部的follow集合
								addTo(followSet[gram->P[i].genL], followSet[str]);//将左部的follow集合加入到当前folow集合
							}
						}
					}//if
			}//for j
		}// for i
		if (str == gram->start) followSet[str].insert("#");//开始符号的终结符加#
	}

	void addToItemSet(string str, ItemSet *itstPtr)
		...
		if (isRDC)//规约填表
		{
			...
			for (; fit != followSet[str].end(); fit++)
			{
				...
				{//表项已经存在无需再填,若存在与将要填写的值不等则出现冲突
					printf("(%d,%d)<-(%d,%d)\n", chart.at(src).at(gram->symPos[*fit]).state, 
												 chart.at(src).at(gram->symPos[*fit]).isReduce, 
												 lrnR.state, lrnR.isReduce);
					flag = false;
				}
				//cout << *fit;
			}//cout << "," << des << ")" << endl;
			if (!flag)
				cerr << "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@reduce collision!\n" << endl;
		}
		else//移进填表
		{
	...
	int makeGo(ItemSet *srcSet, string str, ItemSet *desSet)//扫描Go函数,对项目集中的符号ch做Go
	{//Go函数扫描一个字符到达另一个，state=1表示项目集存在规约
		...
		for (; sit != srcSet->itemset.end(); sit++)//遍历源项目集的每一个项目
		{
			...
			else if (gram->genList[sit->genPos].genR[sit->curPos] == str)//当前字符匹配则移进入下一个
			{
	...
	void showCluster()//显示项目集分析表
	{
		...
		{//输出分析表
			...
			{
				anotherLine++;
				printf("(%3d,%d,%-7s) ", chart.at(i).at(j).state, chart.at(i).at(j).isReduce, order[j].c_str());
